...
def compute_all_data():
    ...
    os.makedirs("../output_forecasts", exist_ok=True)
    ...
    try:
        ...
        # Extract latest ACTUAL yields from raw CSV files (not predictions!)
        latest_yields = {}
        try:
            ...
            for maturity, csv_path in INPUT_FILES.items():
                try:
                    ...
                    if df.empty:
                        print(f"     WARNING: {maturity}Y CSV is empty")
                        continue
                    ...
                    print(f"     WARNING: Could not read {maturity}Y: {e}")
                    continue
            ...
            print(f"  WARNING: Could not extract latest yields: {e}")
            latest_yields = {}
        ...
        print(f"ERROR: Error loading forecasts: {e}")
        import traceback
                ...
                return json.dumps(
                    {
                        ...
                        "bond_type": bond.get("bond_type", "Corporate"),
                        "rating": bond.get("rating", "A"),
            ...
            bonds_info = [
                {
                    ...
                    "bond_type": b.get("bond_type", "Corporate"),
                    "rating": b.get("rating", "A"),
                    "risk_level": b.get("risk_level", "Medium"),
                    "last_traded_price": b.get("last_traded_price"),
                }
                for b in bonds_list
            ]
            result = json.dumps(
    ...
    def handler(self, input_table):
        def get_details(bond_identifier):
            ...
            result = {
                ...
                "maturity_date": bond["maturity_date"],
                "years_to_maturity": round(years_to_maturity, 2),
                # Return both LTP and theoretical price
...
def run_server():
    ...
    if not compute_all_data():
        print("\nERROR: Failed to compute data. Exiting.")
        return
