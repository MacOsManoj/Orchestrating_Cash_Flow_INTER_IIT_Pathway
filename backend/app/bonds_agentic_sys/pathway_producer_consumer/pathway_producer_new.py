...
def get_rdkafka_settings() -> dict:
    ...
    settings = {
        ...
        "auto.offset.reset": "latest",  # Start at the end
        "group.id": f"bond-reader-{uuid.uuid4()}",  # New identity every run
    }
...
class IncrementalRiverTrainer(pw.BaseCustomAccumulator):
    ...
    #         x = {
    #             'lag_1_return': lag1, 'lag_2_return': lag2,
    #             'body': body, 'range': rng,
    #             'avg_body_14': avg_body, 'avg_range_14': avg_rng,
    #             'current_close': close
    #         }
    ...
    def _learn_from_row(self, row):
        ...
        # --- FIX: Helper to safely convert None to 0.0 ---
        def safe_float(v):
        ...
        self.feature_dict = {
            ...
            "last_return": safe_float(lag1),
            "last_lag_1_return": safe_float(lag2),
            "last_body": safe_float(body),
            "last_range": safe_float(rng),
            "last_avg_body_14": safe_float(avg_body),
            "last_avg_range_14": safe_float(avg_rng),
        }

        # 2. Incremental Train (only if we have a valid target)
        if target is not None:
            # --- FIX: Apply safe_float to all training inputs ---
            x = {
                "lag_1_return": safe_float(lag1),
                "lag_2_return": safe_float(lag2),
                "body": safe_float(body),
                "range": safe_float(rng),
                "avg_body_14": safe_float(avg_body),
                "avg_range_14": safe_float(avg_rng),
                "current_close": safe_float(close),
            }
            self.model.learn_one(x, target)
            self.count += 1
...
@pw.udf
def get_predicted_return(data, idx) -> float:
    d = data.as_dict() if hasattr(data, "as_dict") else data
    return (
...
#     base = stream.select(date=parse_date_to_datetime(pw.this.date),
#                          open=pw.this.open,
#                          high=pw.this.high,
#                          low=pw.this.low,
#                          close=pw.this.close,
...
#     basic = sorted_data.with_columns(body=pw.this.close-pw.this.open, range_val=pw.this.high-pw.this.low,
#                                    date_minus_1d=pw.this.date - timedelta(days=1), date_minus_2d=pw.this.date - timedelta(days=2))
...
#     ).select(
#         *pw.left,
#         close_lag_2=pw.right.close
#     )
#     with_rets = with_lags.with_columns(lag_1_return=calculate_return(pw.this.close, pw.this.close_lag_1),
#                                      lag_2_return=calculate_return(pw.this.close_lag_1, pw.this.close_lag_2))

#     daily_win = with_rets.windowby(with_rets.date, window=pw.temporal.sliding(hop=timedelta(days=1), duration=timedelta(days=14)),
#                                  instance=with_rets.maturity, behavior=pw.temporal.exactly_once_behavior())

#     avg_body = daily_win.reduce(window_end=pw.this._pw_window_end, maturity=pw.this._pw_instance, avg_body_14=pw.reducers.avg(pw.coalesce(pw.this.body, 0.0)))
#     avg_range = daily_win.reduce(window_end=pw.this._pw_window_end, maturity=pw.this._pw_instance, avg_range_14=pw.reducers.avg(pw.coalesce(pw.this.range_val, 0.0)))

#     # We use a trick to join window results back: assume window_end aligns with daily processing
#     feats = with_rets.with_columns(
#             daily_key=pw.apply_with_type(
#                 lambda dt: dt.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1),
#                 pw.DateTimeNaive, # <--- Explicitly specify return type
#                 pw.this.date
#             )
#         ).join(
#             avg_body,
#             pw.this.daily_key == avg_body.window_end,
#             how=JoinMode.LEFT
#         ).select(
#             *pw.left,
#             avg_body_14=pw.right.avg_body_14
#         ).join(
#             avg_range,
#             pw.this.daily_key == avg_range.window_end,
#             how=JoinMode.LEFT
#         ).select(
#             *pw.left,
#             avg_range_14=pw.right.avg_range_14
#         )
#     # final = feats.filter(
...
#     #     )
#     # )
#     feats_with_target = feats.with_columns(date_plus_1d=pw.this.date + timedelta(days=1))

#     # Get next close to calculate target return
#     training_set = feats_with_target.asof_join(
#         sorted_data, feats_with_target.date_plus_1d, sorted_data.date, how=JoinMode.LEFT
#     ).select(
#         *pw.left,
#         next_close=pw.right.close
#     ).with_columns(
#         target_return = calculate_return(pw.this.next_close, pw.this.close)
#     )

#     final = training_set.filter(
#         pw.this.close.is_not_none() & pw.this.lag_1_return.is_not_none() & pw.this.lag_2_return.is_not_none() &
#         pw.this.body.is_not_none() & pw.this.range_val.is_not_none() & pw.this.avg_body_14.is_not_none() &
#         pw.this.avg_range_14.is_not_none()
#     )
...
def build_hybrid_feature_pipeline(stream):
    ...
    # Remove null dates immediately
    base = base.filter(pw.this.date.is_not_none())

    # FIX 1: Remove "sorted_data = base.sort(...) + base"
    # That line duplicated every row! 'base' is sufficient for streaming.

    # Calculate basic features
    basic = base.with_columns(
    ...
    # Self-joins for lags (using base instead of sorted_data)
    with_lag1 = basic.asof_join(
        base, basic.date_minus_1d, base.date, how=JoinMode.LEFT
    ).select(*pw.left, close_lag_1=pw.right.close)

    with_lags = with_lag1.asof_join(
        base, with_lag1.date_minus_2d, base.date, how=JoinMode.LEFT
    ).select(*pw.left, close_lag_2=pw.right.close)

    # ... [Keep your window/rolling average logic exactly as is] ...
    # (Copying your existing logic for brevity, it was fine)
    ...
    # )
    # NEW CODE: Handle the case where the entire average is None
    avg_body = daily_win.reduce(
        window_end=pw.this._pw_window_end,
        maturity=pw.this._pw_instance,
        # Calculate avg, then if result is None, become 0.0
        avg_body_14=pw.coalesce(pw.reducers.avg(pw.this.body), 0.0),
    )

    avg_range = daily_win.reduce(
        window_end=pw.this._pw_window_end,
        maturity=pw.this._pw_instance,
        avg_range_14=pw.coalesce(pw.reducers.avg(pw.this.range_val), 0.0),
    )
    # Join stats back
    feats = (
    ...
    # Get Target for training (Next Day Close)
    ...
    models = final.groupby(final.maturity).reduce(
        maturity=final.maturity,
        date=pw.reducers.max(final.date),
        model=incremental_model_reducer(
            final.date,
            final.close,
            final.lag_1_return,
...
#     kafka_table = pw.Table.empty(SYMBOL=str, ISIN=str, **{"ISIN Description": str, "Maturity Date": str, "FACE VALUE": float}, LTP=float)
...
#     tables = []
#     kafka_yields = pw.io.kafka.read(
#                 rdkafka_settings=rdkafka_settings,
#                 topic=KAFKA_TOPIC_RAW_YIELDS,
#                 format="json",
#                 schema=KafkaYieldSchema,
...
#             stream = pw.io.python.read(YieldStreamSubject(mat, fname, 0.0001), schema=YieldSchema, autocommit_duration_ms=100)

#             kafka_sub = kafka_yields.filter(pw.this.maturity == mat)
#             pw.debug.compute_and_print(kafka_sub)
#             # kafka_sub = pw.Table.empty(date=str, open=float, **{"high": float, "low": float, "close": float}, maturity=int)
...
#         for t in tables[1:]: combined_forecasts = combined_forecasts.concat_reindex(t)
#         # Unpack JSON forecasts into rows
#         output_rows = []
#         for d in range(FORECAST_DAYS):
#             output_rows.append(combined_forecasts.select(
...
#                 Predicted_Return=get_predicted_return(pw.this.forecast_data, d) # <--- ADDED COLUMN HERE
#             ))

#         final_output = output_rows[0]
#         for r in output_rows[1:]: final_output = final_output.concat_reindex(r)
...
def main():
    ...
    for mat, fname in INPUT_FILES.items():
        # Get historical max date for this maturity
        df = pd.read_csv(fname)
        try:
        ...
        logger.info(f"Processing {mat}Y: History until {last_ts}")

        if os.path.exists(fname):
            # Read CSV Stream (History)
            csv_stream = pw.io.python.read(
                YieldStreamSubject(mat, fname, 0.0001),
                schema=YieldSchema,
                autocommit_duration_ms=100,
            )

            # Filter Kafka for this maturity
            kafka_sub = kafka_yields.filter(pw.this.maturity == mat)

            # Combine History + Live Kafka
            combined_stream = pw.Table.concat_reindex(csv_stream, kafka_sub)

            # Build Models
            model_table = build_hybrid_feature_pipeline(combined_stream)

            # --- FIX 2: THE CRITICAL CHANGE ---
            # Instead of == last_ts, we want updates NEWER than history start
            # or just take ALL updates. The model_table emits whenever the model updates.
            # We filter for dates >= last_ts so we don't output 10 years of back-forecasts
            last_ts_midnight = last_ts.replace(
                hour=0, minute=0, second=0, microsecond=0
            )
            latest_updates = model_table.filter(pw.this.date >= last_ts)

            # Generate Forecasts
            forecast_table = latest_updates.select(
                maturity=pw.this.maturity,
                forecast_date=pw.this.date,
                forecast_data=generate_forecast(pw.this.model, pw.this.maturity),
            )
    ...
    if tables:
        ...
        final_output = final_output.filter(final_output.Predicted_Yield != 0.0)

        # Print debug info to console to prove it's working

        # Write to CSV (Streaming Mode)
        # This will create the file and Append new rows as they arrive
        pw.io.csv.write(final_output, os.path.join(OUTPUT_DIR, "final_forecasts.csv"))

        logger.info("Pipeline built. Listening for Kafka updates...")
        pw.run()  # <--- This will now block and process forever
    else:
        logger.error("No input files found.")
