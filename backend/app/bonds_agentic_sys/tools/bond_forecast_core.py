...
# Use relative paths from project root (not hardcoded absolute paths)
# Get project root (bond-pipeline directory)
PROJECT_ROOT = Path(__file__).parent.parent
DATA_DIR = PROJECT_ROOT / "data"

INPUT_FILES = {
    1: str(DATA_DIR / "1y.csv"),
    2: str(DATA_DIR / "2y.csv"),
    5: str(DATA_DIR / "5y.csv"),
    7: str(DATA_DIR / "7y.csv"),
    10: str(DATA_DIR / "10y.csv"),
}

OUTPUT_DIR = str(PROJECT_ROOT / "output_forecasts")
os.makedirs(OUTPUT_DIR, exist_ok=True)

BONDS_CSV_FILE = str(DATA_DIR / "bonds_data.csv")
...
def load_bonds_from_csv(csv_path):
    ...
    print(f"  Warning: {len(bonds_list) - len(bonds_with_ltp)} bonds missing LTP")
...
def filter_bonds_by_criteria(bonds_list, criteria):
    ...
    for bond in bonds_list:
        ...
        # Filter by maturity year if specified (e.g., "bonds mature by 2032")
        if "maturity_year" in criteria:
            target_year = criteria["maturity_year"]
            if maturity_year > target_year:
                continue

        if "max_maturity_year" in criteria:
            max_year = criteria["max_maturity_year"]
            if maturity_year > max_year:
                continue

        if "min_maturity_year" in criteria:
            min_year = criteria["min_maturity_year"]
            if maturity_year < min_year:
                continue

        # Also support years_to_maturity filtering (for backward compatibility)
