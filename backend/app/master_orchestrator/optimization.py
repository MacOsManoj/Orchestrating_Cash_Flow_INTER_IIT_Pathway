...
from backend.app.cash_flow.liquidity_risk_tools import predict_liquidity_regime
from backend.app.cash_flow.market_regime import get_market_regime_simplified
...
def get_rbi_policy_rates() -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            text = soup.get_text()
            summary_match = re.search(r'Cash Reserve Ratio in India.*?(\d+\.\d+)\s+percent', text, re.IGNORECASE)
            if summary_match:
...
def calculate_portfolio_breakdown(investment_ratio, base_slr, market_state, preference):
    ...
    if preference == "Aggressive":
        if market_state == "High":     weights = {"Stocks": 0.70, "CorpBonds": 0.20, "Forex": 0.10, "GSec": 0.00}
        elif market_state == "Medium": weights = {"Stocks": 0.50, "CorpBonds": 0.30, "Forex": 0.10, "GSec": 0.10}
        else:                          weights = {"Stocks": 0.30, "CorpBonds": 0.40, "Forex": 0.10, "GSec": 0.20}
        
    elif preference == "Safe":
        if market_state == "High":     weights = {"Stocks": 0.20, "CorpBonds": 0.40, "Forex": 0.05, "GSec": 0.35}
        else:                          weights = {"Stocks": 0.05, "CorpBonds": 0.20, "Forex": 0.05, "GSec": 0.70}
        
    else: # Normal
        if market_state == "High":     weights = {"Stocks": 0.50, "CorpBonds": 0.30, "Forex": 0.10, "GSec": 0.10}
        elif market_state == "Medium": weights = {"Stocks": 0.30, "CorpBonds": 0.40, "Forex": 0.10, "GSec": 0.20}
        else:                          weights = {"Stocks": 0.10, "CorpBonds": 0.40, "Forex": 0.10, "GSec": 0.40}
...
def determine_allocation_ratios():
    ...
    if market_state == "High":     ir_buffer = 0.20
    elif market_state == "Medium": ir_buffer = 0.10
    else:                          ir_buffer = 0.02
...
if __name__ == "__main__":
    ...
    print("="*60)
    print(f"CONDITIONS: Liquidity={result['states']['liquidity']} | Market={result['states']['market']}")
    print("-" * 60)
    print(f"1. CASH RESERVE (RR):   {r['RR']:.2%} (Min CRR: {result['rbi']['CRR']:.2%})")
    print(f"2. LOAN BOOK (LR):      {r['LR']:.2%}")
    print(f"3. INVESTMENTS (IR):    {r['IR']:.2%} (Min SLR: {result['rbi']['SLR']:.2%})")
    print("-" * 60)
