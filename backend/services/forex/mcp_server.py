...
The server will be available at: http://localhost:8127/mcp/
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8127"))
...
def get_trades_summary_impl(pairs: List[str]) -> str:
    ...
    if not trades_data:
        return json.dumps(
            {
                "error": True,
                "message": "trades.json not found. No trade history available.",
            }
        )
    ...
    if not valid_pairs:
        return json.dumps(
            {
                "error": True,
                "message": f"No valid pairs provided. Valid pairs are: {FOREX_PAIRS}",
            }
        )
        ...
        if pair in trades_data:
            ...
            results[pair] = {
                ...
                "avg_holding_period_days": round(
                    data.get("avg_holding_period_days", 0), 1
                ),
                "recent_trades": [
                    {
            ...
            results[pair] = {
                "error": True,
                "message": "No trade data available for this pair",
            }
...
def get_position_details_impl(pairs: List[str]) -> str:
    ...
    if not positions_data:
        return json.dumps(
            {
                "error": True,
                "message": "positions.json not found. No position data available.",
            }
        )
        ...
        if pair in positions_data:
            ...
            results[pair] = {
                ...
                "unrealized_pnl_pct": round(
                    data.get("unrealized_pnl_pct", data.get("last_trade_pnl_pct", 0)), 2
                ),
                ...
                "days_held": data.get(
                    "days_held", data.get("days_since_last_trade", 0)
                ),
    ...
    return json.dumps(
        {"error": False, "data": results, "portfolio_summary": portfolio_summary}
    )
...
def get_currency_regime_impl(pairs: List[str]) -> str:
    ...
    for pair in valid_pairs:
        ...
        if df.empty:
            results[pair] = {
                "error": True,
                "message": f"Data file not found for {pair}",
            }
            continue
        ...
        try:
            ...
            if hurst > 0.6:
                ...
                strategy_recommendation = (
                    "Mixed approach or reduced position size recommended"
                )
            ...
            results[pair] = {
                "error": True,
                "message": f"Error analyzing regime: {str(e)}",
            }
...
def get_currency_correlation_impl(pairs: List[str]) -> str:
    ...
    if len(valid_pairs) < 2:
        return json.dumps(
            {
                "error": True,
                "message": "Need at least 2 valid pairs to calculate correlation.",
            }
        )
    ...
    if len(returns_dict) < 2:
        return json.dumps(
            {
                "error": True,
                "message": "Could not load sufficient data for correlation analysis.",
            }
        )
...
def get_news_sentiment_impl(pairs: List[str]) -> str:
    ...
    try:
        ...
        return json.dumps(
            {"error": True, "message": f"Failed to initialize news tool: {str(e)}"}
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Available tools: get_trades_summary, get_position_details, get_currency_regime, get_currency_correlation, get_news_sentiment"
    )
